...
void loop(void)
{
  ...
  bool res = false;
  if (songNo == 1) res = play_rtttl(song_1);
  else if (songNo == 2) res = play_rtttl(song_2);
  else if (songNo == 3) res = play_rtttl(song_3);
  else if (songNo == 4) res = play_rtttl(song_4);
  else if (songNo == 5) res = play_rtttl(song_5);
  else if (songNo == 6) res = play_rtttl(song_6);
  else if (songNo == 7) res = play_rtttl(song_7);

  if (res) {
    while (digitalRead(buttonPin)) {} // wait till it drops to LOW
    songNo++;
    if (songNo > 7) songNo = 1;
  }
}
